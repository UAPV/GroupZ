...
class uapvLdap
{
  ...
  public function connect ()
  {
    ...
    if (array_key_exists('options', $this->config) && is_array($this->config['options']))
      foreach ($this->config ['options'] as $k => $v)
        if (! @ldap_set_option ($this->ldapCon, $k, $v))
          throw new sfException ("Unable to set ldap option ($k = $v) : ".ldap_error ($this->ldapCon));
    ...
    if ($binded === false)
      throw new sfException ("Cannot bind to the ldap server $host:$port with dn '$dn' : ".ldap_error ($this->ldapCon));
  }
  ...
  public function disconnect ()
  {
      if (is_resource ($this->ldapCon) && ! @ldap_unbind ($this->ldapCon))
          throw new sfException ('Echec de la déconnexion du serveur ldap : '.ldap_error ($this->ldapCon));
   ...
   */
  public function search ($filter = null, $sizelimit = 0, $dnPrefix = null, $justthese = null, $attrsonly = 0)
  {
      $this->connect ();
